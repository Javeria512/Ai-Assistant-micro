/**
 * Microsoft sign-in and the encrypted session store behind it.
 *
...
import type { SessionUser, TokenResponse } from '../models/api';
...
/** Fallback lifetime when the redirect omits `expires_in`, in seconds. */
...
export function parseAuthDeepLink(url: string): StoredSession | null {
  ...
  const expiresIn = Number(params.get('expires_in') ?? DEFAULT_EXPIRY_S);
  return {
    accessToken,
    refreshToken,
    expiresAt:
      Date.now() +
      (Number.isFinite(expiresIn) ? expiresIn : DEFAULT_EXPIRY_S) * 1000,
    user: null,
  };
}
